<nav class="navbar">
    <div class="container">
        <a href="{{ route('home') }}" class="navbar-logo">
            <img src="{{ asset('assets/logo.png') }}" alt="AVA VPN">
            <span>AVA <em>VPN</em></span>
        </a>
        <div class="navbar-actions">
            @php
                $botUsername = (string) (config('services.telegram_bot.username') ?? '');
                $botUrl = $botUsername !== '' ? 'https://t.me/'.ltrim($botUsername, '@') : null;
                $channelUrl = (string) config('app.telegram_channel_url');
            @endphp
            @isset($visitorCount)
                <button type="button" class="visitor-counter-btn" id="visitor-counter-trigger" aria-label="{{ __('landing_traffic.counter_aria') }}" title="{{ __('landing_traffic.counter_hint') }}">
                    <table class="visitor-counter"><tr><td>{{ number_format((int) $visitorCount, 0, '.', ' ') }}</td></tr></table>
                </button>
            @endisset
            @if ($channelUrl !== '')
                <a href="{{ $channelUrl }}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm">ТГ-канал</a>
            @endif
            @if ($botUrl)
                <a href="{{ $botUrl }}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm">ТГ-бот</a>
            @endif
            <a href="{{ config('app.telegram_support_url') }}" target="_blank" rel="noopener" class="navbar-tg">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                <span>Задать вопрос</span>
            </a>
            @auth
                <a href="{{ route('cabinet.subscription') }}" class="btn btn-primary btn-sm">Кабинет</a>
            @else
                <a href="{{ route('login') }}" class="btn btn-primary btn-sm">Войти</a>
            @endauth
        </div>
    </div>
</nav>
